  ...
  return (
    <div className={`relative ${className}`} ref={ref}>
      ...
      {open && (
        <ul
          role="listbox"
          className="absolute right-0 top-full mt-1 z-50 min-w-[180px] bg-sg-bg-elevated border border-sg-border rounded-card py-1 shadow-lg shadow-black/30 animate-in"
        >
          {CHAINS.map((chain) => (
            <li key={chain.id}>
              <button
